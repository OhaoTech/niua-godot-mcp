...
| `get_output_logs` | v1, full | Read recent Godot bridge logs, runtime log events, and stdout/stderr captured from local Godot editor processes launched by this MCP server. | host:string, port:number, expectedProjectRoot:string, projectId:string, projectRoot:string, includeBridge:boolean, includeProcess:boolean, maxLines:number |
