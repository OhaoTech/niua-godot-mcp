...
| --- | --- | --- | --- |
| `list_export_presets` | full | List Godot export presets configured in export_presets.cfg. | host:string, port:number |
| `upsert_export_preset` | full | Create or update a Godot export preset through the visible editor bridge. | host:string, port:number, index:number, name*:string, platform*:string, exportPath:string, runnable:boolean, exportFilter:string, includeFilter:string, excludeFilter:string, customFeatures:string, dedicatedServer:boolean, options:object |
