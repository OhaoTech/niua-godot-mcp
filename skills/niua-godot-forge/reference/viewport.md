...
| --- | --- | --- | --- |
| `capture_editor_screenshot` | full | Capture a PNG screenshot of the visible Godot editor. Returns available=false when the active renderer cannot expose editor pixels, such as headless mode. | host:string, port:number, expectedProjectRoot:string |
| `capture_viewport_screenshot` | full | Capture a PNG screenshot from the Godot editor 2D or 3D viewport. Returns available=false when the active renderer cannot expose viewport pixels, such as headless mode. | host:string, port:number, expectedProjectRoot:string, viewport:string, index:number |
| `get_viewport_state` | full | Read Godot editor 2D or 3D viewport size and active camera metadata when exposed by the editor. | host:string, port:number, expectedProjectRoot:string, viewport:string, index:number |
