...
| `get_scene_tree` | v1, full | Read the current scene tree from the visible Godot editor. Defaults to maxDepth 2; pass 0 for the full tree, pathFilter for a subtree, or savePath to keep the full JSON on disk. | host:string, port:number, expectedProjectRoot:string, maxDepth:number, pathFilter:string, savePath:string |
