...
| `configure_hdr_output` | full | Enable or disable Godot 4.7 HDR monitor output and optional HDR 2D via Project Settings. | host:string, port:number, expectedProjectRoot:string, requestHdrOutput:boolean, hdr2d:boolean, save:boolean |
...
| `get_input_map` | full | Read the project's input actions from ProjectSettings — the same source set_input_action writes and the running game loads. Actions list in sorted name order; built-in ui_* actions appear only if the project overrides them. | host:string, port:number, expectedProjectRoot:string |
