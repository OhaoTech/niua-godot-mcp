...
test("Godot debugger probe delegates focused plugin domains", async () => {
  ...
  assert.doesNotMatch(state, /store\.runtime_events\(\)/);
  assert.doesNotMatch(sessions, /lastRuntimeMessage/);
  ...
  assert.match(runtimeRequests, /func send_runtime_node_properties_request\(debugger_probe: EditorDebuggerPlugin, session_ids: Array\[int\], node_path: String, request_id: String, record_event: Callable, properties: Array = \[\], verbose: bool = false\) -> Array:/);
